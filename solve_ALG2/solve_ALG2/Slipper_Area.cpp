...
void ALG2_Slipper::create_f_matrix() 
{
    std::cout << "Создание матрицы f1, f2" << "; Размер f1: " << (nx1 + 1 + nx2 + 1) << " x " << (ny1 + 1) << "Размер f2: " << (nx1 + 1) << " x " << (ny2) << std::endl;
...
ALG2_Slipper::ALG2_Slipper(double hx, double hy, double Lx1, double Lx2, double Ly1, double Ly2, double r, double mu, double tau_s, double alpha, const std::string& filename)
{
    ...
    f1 = std::vector<std::vector<double>>(nx1 + 1 + nx2, std::vector<double>(ny1 + 1, 0.0));       // Основная переменная (потенциал) (размер: (nx1 + 1 +  nx2 + 1) x ( ny1 + 1 )
    f2 = std::vector<std::vector<double>>(nx1 + 1, std::vector<double>(ny2, 0.0));       // Основная переменная (потенциал) (размер: (nx1 + 1) x (ny2)
...
double ALG2_Slipper::U_ij(double u_ij, double u_ip1j, double u_ijp1, double u_im1j, double u_ijm1, double f_ij)
{
    return (this->sm2 * u_ij + u_ip1j + u_im1j + u_ijp1 + u_ijm1 + f_ij * this->h2) * this->osp2;
}
double ALG2_Slipper::Error(double u_ij, double u_ip1j, double u_ijp1, double u_im1j, double u_ijm1, double f_ij)
{
    return fabs(-4.0 * u_ij + u_ip1j + u_im1j + u_ijp1 + u_ijm1 + f_ij * this->h2);
}

double ALG2_Slipper::Error_Area(const std::vector<std::vector<double>>& data1, const std::vector<std::vector<double>>& data2)
{
...
void ALG2_Slipper::PoissonSolver2D(double EPS, int max_iter)
{
...
void  ALG2_Slipper::update_Poisson()
{
    ...
    for (int i = 1; i < nx1 + nx2; ++i)
        for (int j = 1; j < ny1; j++)
            this->F1[i][j] =  (f1[i][j] + this->div_lambda1[i-1][j-1] - this->r * this->div_q1[i-1][j-1]) / this->r; // f1[i][j] / this-> r;
...
void ALG2_Slipper::divergence(std::vector<std::vector<double>>& field11, std::vector<std::vector<double>>& field12, std::vector<std::vector<double>>& div1, std::vector<std::vector<double>>& field21, std::vector<std::vector<double>>& field22, std::vector<std::vector<double>>& div2) 
{
    for (int i = 1; i < nx1 + nx2; ++i)
        for (int j = 1; j < ny1; ++j) 
...
void ALG2_Slipper::calculation_t(std::vector<std::vector<double>>& lamda, std::vector<std::vector<double>>& grad_v, std::vector<std::vector<double>>& t, std::vector<std::vector<double>>& t_extended, int Nx, int Ny, int k, int l)
{
    for (int i = 0; i < Nx; ++i)
        for (int j = 0; j < Ny; ++j)
...
void ALG2_Slipper::calculation_norma(std::vector<std::vector<double>>& t, std::vector<std::vector<double>>& t_avg, std::vector<std::vector<double>>& norma, int Nx, int Ny)
{
    for (int i = 0; i < Nx; i++)
        for (int j = 0; j < Ny; j++)
...
void ALG2_Slipper::calculation_q(std::vector<std::vector<double>>& t, std::vector<std::vector<double>>& norma, std::vector<std::vector<double>>& q,  int Nx, int Ny)
{
    for (int i = 0; i < Nx; ++i)// {
        for (int j = 0; j < Ny; ++j) //{
...
void ALG2_Slipper::calculation_lamda(std::vector<std::vector<double>>& lambda, std::vector<std::vector<double>>& grad_v, std::vector<std::vector<double>>& q, int Nx, int Ny)
{
...
void ALG2_Slipper::update_lambda() {
    std::cout << std::endl << "============================================================" << std::endl;
...
int ALG2_Slipper::run_full_algorithm(int max_iterations, double convergence_tol) {
...
void ALG2_Slipper::Save(int bol)
{
    if (bol == 1)
    { 
...
void ALG2_Slipper::check_symmetry_detailed() {
...
void ALG2_Slipper::print_iteration_time() {
